...
-- =============================================
create PROCEDURE [prj].[teleminder_V2_Part2]
...
WITH teleminder AS
(
 SELECT e.[First Name   ] AS firstname,e.[Last Name   ] AS lastname,e.[Patient ID   ] AS act_nbr,e.[Phone Number   ] AS DayPhone
		FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0',
		  'Excel 12.0;Database=C:\temp\teleminder_xlsx\CopyofBrooksideTimedOutcalls.xls', [Sheet1$]) AS e
)SELECT REPLACE(REPLACE(t.firstname,'-',' '),'"','') AS first_name,
  REPLACE(REPLACE(t.lastname,'-',' '),'"','')  AS last_name,
  REPLACE(REPLACE(REPLACE(replace(STR(t.DayPhone,25,0),'-',''),' ',''),'(',''),')','') AS day_phone,
  STR(t.act_nbr,25,0) AS MED_REC_NBR
 INTO #tempTable
