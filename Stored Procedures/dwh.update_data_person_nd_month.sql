...
AS
   BEGIN
        ...
        SELECT 
                ...
                per.deceased,
                0 AS ng_data
        FROM [sqlprod1\ghost].Prod_Ghost.dwh.data_patient_ecw per
        ...
        FROM    [10.183.0.94].NGProd.dbo.person per
                ...
                LEFT JOIN [10.183.0.94].NGProd.dbo.patient pat ON per.person_id = pat.person_id
                LEFT JOIN [sqlprod1\ghost].Prod_Ghost.dwh.data_location loc ON loc.ud_demo3_id = ud.ud_demo3_id
        ...
        SELECT
                ...
                loc.location_key,
                0 AS ng_data
        FROM [sqlprod1\ghost].Prod_Ghost.dwh.data_patient_ecw pat
        LEFT JOIN [sqlprod1\ghost].Prod_Ghost.dwh.data_location loc ON loc.ecw_location_id = pat.med_home_id
        ...
        SELECT  per.person_id AS person_id ,
                ...
                1 AS ng_data
        FROM    [10.183.0.94].NGProd.dbo.person per
                LEFT JOIN [sqlprod1\ghost].Prod_Ghost.dwh.data_user pro ON per.primarycare_prov_id = pro.provider_id
        ...
        SELECT
            ...
            0 AS ng_data
        FROM [sqlprod1\ghost].Prod_Ghost.dwh.data_patient_ecw pat
        LEFT JOIN [sqlprod1\ghost].Prod_Ghost.dwh.data_provider prov ON prov.ecw_provider_key = pat.pcp_id
        ...
        FROM    #person_patient pp
                ...
                CROSS JOIN ( SELECT DISTINCT
                                    first_mon_date
                             FROM   [sqlprod1\ghost].Prod_Ghost.dwh.data_time
        ...
        SELECT  nr.person_id ,
                ...
                CASE WHEN nr.pcp_cur_key != ( SELECT TOP 1  -- yes I wish I didn't have to use top, but the DQ issues are so prevelant that this is a work around.
                                                        user_key
                                              FROM      [sqlprod1\ghost].Prod_Ghost.dwh.data_user
        ...
        SELECT  nr.person_id ,
                ...
                CASE WHEN
                         nr.mh_cur_key != ( SELECT TOP 1
                                                    location_key
                                             FROM   [sqlprod1\ghost].Prod_Ghost.dwh.data_location
        ...
        SELECT  IDENTITY( INT, 1, 1 )  AS per_mon_id ,
                ...
                nr.med_rec_nbr,
                nr.ng_data
        INTO    Staging_Ghost.dwh.data_person_nd_month
        FROM    #ND_roll_up nr
        ...
        ALTER TABLE Staging_Ghost.dwh.data_person_nd_month
        ADD CONSTRAINT per_mon_id_pk PRIMARY KEY (per_mon_id);

        CREATE NONCLUSTERED INDEX IX_person_mon_x_first_mon_date2
        ON Staging_Ghost.dwh.data_person_nd_month (per_mon_id, first_mon_date);
