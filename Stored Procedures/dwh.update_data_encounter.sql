...
AS
 BEGIN
	...
	-- interfering with SELECT statements.
        ...
        WITH    qual_enc
                ...
                enc_data
                  AS ( SELECT   enc.enc_id ,
                                ...
                                ( SELECT TOP 1
                                            location_key
                                  FROM      [sqlprod1\ghost].Prod_Ghost.dwh.data_location loc
                                  WHERE     loc.location_id_unique_flag = 1
                                            AND loc.location_id = enc.location_id
                                ) AS location_key ,
                                ( SELECT TOP 1
                                            user_key
                                  FROM      [sqlprod1\ghost].Prod_Ghost.dwh.data_user prov
                                ...
                                ( SELECT TOP 1
                                            per_mon_id
                                  FROM      staging_ghost.dwh.data_person_nd_month per
        ...
        ALTER TABLE Staging_Ghost.dwh.data_encounter
